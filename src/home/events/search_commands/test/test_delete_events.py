...
class DeleteTests(APITestCase):
    ...
    def test_delete_events_everything_else_worked(self):
    ...
    def test_delete_events_deletes_events(self):
        """
        """
        ...
        query = Query(
            name="test",
            text="search index=test | delete_events",
            user=self.user,
        )
        results = query.resolve(
            request=MagicMock(user=self.user),
