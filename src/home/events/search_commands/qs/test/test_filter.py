...
command, located at events.search_commands.qs.filter.
"""
import json
...
class FilterTests(APITestCase):
    ...
    def test_filter(self):
        """Test the filter command to ensure it correctly filters
        the records from the QuerySet based on the provided expressions.
        """
        query = Query(
            name="test",
            text="search index=test | qs_filter extracted_fields__bar=1",
            user=self.user,
        )
