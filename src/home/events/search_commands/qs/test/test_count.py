"""This test module is meant to test the count search
command, located at events.search_commands.qs.count.
"""
import json
...
class CountTests(APITestCase):
    ...
    def test_count(self):
        """Test the count command to ensure it correctly counts
        the number of records in the QuerySet.
        """
        query = Query(
            name="test",
            text="search index=test | qs_count",
            user=self.user,
        )
